...
def _is_secret_like_key(key: str) -> bool:
    ...
    if not words:
        return False
    if any(word in {"secret", "secrets", "password", "passwd"} for word in words):
        return True
    if words in {
        ("credential",),
        ("credentials",),
    ...
    if pairs.intersection(
        ...
        return True
    return len(words) == 1 and words[0] in {"token", "key"}
...
class ManifestMetadata(_ManifestModel):
    ...
    @classmethod
    def validate_principal_or_cost_field(cls, value: Any, info) -> str:
        return _clean_text(
            value,
...
class EnvironmentSpec(_ManifestModel):
    ...
    @classmethod
    def clean_optional_resource_fields(cls, value: Any, info) -> Any:
        if value is None:
            return None
...
class ResourceBindings(_ManifestModel):
    ...
    @classmethod
    def validate_optional_resource_ids(cls, value: Any, info) -> Any:
        if value is None:
            return None
        return _validate_resource_reference(value, field=info.field_name)

    @field_validator("evaluation_job_id", "promotion_job_id")
    @classmethod
    def validate_databricks_job_ids(cls, value: str | None, info) -> str | None:
        if value is None:
            return None
        if (
            not value.isascii()
    ...
    @classmethod
    def normalize_optional_job_keys(cls, value: Any, info) -> Any:
        if value is None:
            return None
        return _snake_identifier(value, field=info.field_name)
    ...
    @classmethod
    def normalize_reference_arrays(cls, value: Any, info) -> tuple[str, ...]:
        values = _as_tuple(value, field=info.field_name)
        normalized = tuple(
            _validate_resource_reference(item, field=info.field_name) for item in values
        )
...
class AIApplicationManifest(_ManifestModel):
    ...
    api_version: Literal[MANIFEST_API_VERSION] = Field(alias="apiVersion")
    kind: Literal[MANIFEST_KIND]
    ...
    def validate_controlled_tags(self) -> Self:
        ...
        for environment_name, environment in self.spec.environments.items():
            ...
            if effective.get("lifecycle", "experimental") not in {
                "experimental",
                "production",
                "retired",
            }:
                raise ValueError(
                    ...
                    "experimental, production, or retired"
                )
        return self

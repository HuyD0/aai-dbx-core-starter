...
class MeanConfidence(ContractModel):
    ...
    method: Literal["normal-mean-v1"] = "normal-mean-v1"
...
class PairedConfidence(ContractModel):
    ...
    confidence_level: float = Field(gt=0.5, lt=1.0)
    method: Literal["paired-normal-mean-v1"] = "paired-normal-mean-v1"
...
def build_statistical_evidence(
    ...
    for metric in sorted(samples):
        ...
        if not values:
            continue
        mean, lower, upper = _mean_interval(values, config.confidence_level)
        ...
        if not improvements:
            continue
        paired_mean, paired_lower, paired_upper = _mean_interval(
            improvements, config.confidence_level
        )
